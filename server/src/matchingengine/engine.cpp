...
namespace orderbook
{
    ...
    {
        const auto request =
            deserialize<core::protocol::view::GetBookRequest>(payload);

        BOOST_LOG_TRIVIAL(debug)
            << "Connection ID=" << payload.connection_id << ", " << request;
    ...
    void Engine::handle_add_order_request(
        const core::Payload& payload, const core::Timestamp timestamp
    )
    {
        const auto request =
            deserialize<core::protocol::trading::AddOrderRequest>(payload);
        ...
        auto [order_id, trades] = book.add_order(
            request.price(), request.quantity(), request.side(), timestamp
        );

        auto outbound_payload = serialize(
            payload.connection_id,
            core::protocol::trading::AddOrderResponse(
                request.symbol(), order_id, request.price(), request.quantity(),
        ...
        for (const auto& trade : trades) {
            outbound_payload = serialize(
                payload.connection_id,
                core::protocol::trading::Trade(
                    request.symbol(), trade.id(), trade.price(),
                    trade.quantity(), trade.timestamp(), trade.source_order(),
                    trade.matched_order()
                )
            );
            if (!f_outbound_buffer.push(outbound_payload)) {
    ...
    void Engine::handle_modify_order_request(
        const core::Payload& payload, const core::Timestamp timestamp
    )
    {
        const auto request =
            deserialize<core::protocol::trading::ModifyOrderRequest>(payload);
        ...
        const auto success = book.modify_order(
            request.order_id(), request.quantity(), timestamp
        );

        const auto outbound_payload = serialize(
            payload.connection_id, core::protocol::trading::ModifyOrderResponse(
    ...
    {
        const auto request =
            deserialize<core::protocol::trading::CancelOrderRequest>(payload);
