...
void assert_iterable(
    std::initializer_list<T> expected, const std::vector<T>& actual,
    CheckFn check
)
{
...
void assert_iterable(
    const Iterable& expected, const Iterable& actual, CheckFn check
)
{
    ASSERT_EQ(
    ...
    for (; it_exp != std::end(expected); ++it_exp, ++it_act) {
        if constexpr (is_iterable_v<Elem> &&
                      !is_map_v<Elem>) { // Handle nested vectors, etc.
            // Let template deduction figure out the types
            // automatically.
...
void assert_map(const Map& expected, const Map& actual, CheckFn check)
{
    ...
    for (const auto& [key, expected_val] : expected) {
        ...
        ASSERT_TRUE(it != actual.end())
            << "Key not found in actual map: " << key;
