...
namespace tcp
{
    ...
    public:
        ...
        template <typename SubMessage> void request(const SubMessage& request);
    ...
    void Client::register_handler(ResponseHandler<SubMessage> handler)
    {
        ...
        f_handlers[SubMessage{}.type()] =
            [h = std::move(handler)](const std::vector<unsigned char>& buffer) {
                SubMessage msg{};
    ...
    void Client::request(const SubMessage& request)
    {
        ...
        core::protocol::Header::serialize(
            {request.type(), request.size()}, buffer->data()
        );
        request.serialize(buffer->data() + core::protocol::Header::Size);

        BOOST_LOG_TRIVIAL(debug)
            << "async_write: scheduling write of " << buffer_size << " bytes";
            ...
            [self = shared_from_this(
                ...
                if (ec) {
                    ...
                    BOOST_LOG_TRIVIAL(info)
                        << "async_write completed, wrote " << bytes << " bytes";
                }
            }
        );
    }
