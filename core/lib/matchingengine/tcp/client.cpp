        ...
        boost::asio::async_read(
            ...
            [self = shared_from_this()](
                ...
                BOOST_LOG_TRIVIAL(info)
                    << "Header read completed,  " << bytes << " bytes read";
                ...
                BOOST_LOG_TRIVIAL(debug)
                    << "Header received: type=" << static_cast<int>(header.type)
    ...
    void Client::read_payload(const core::protocol::Header& header)
    {
        ...
        boost::asio::async_read(
            ...
            [self = shared_from_this(), header](
                ...
                BOOST_LOG_TRIVIAL(info)
                    << "Payload read completed,  " << bytes << " bytes read";
                ...
                try {
                    BOOST_LOG_TRIVIAL(debug)
                        << "Trying to deserialize message of type: "
                        << header.type;
                    ...
                    BOOST_LOG_TRIVIAL(error)
                        << "Deserialize or Handler error: " << e.what();
                } catch (...) {
                    BOOST_LOG_TRIVIAL(error)
                        << "Deserialize or Handler error: unknown";
                }
