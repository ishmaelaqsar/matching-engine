...
{
    const auto tables = {
        ui->bidsTable, ui->asksTable, ui->ordersTable, ui->tradesTable
    };
...
{
    connect(
        ui->actionConnect, &QAction::triggered, this,
        &MainWindow::onActionConnectTriggered
    );
    connect(
        ui->actionDisconnect, &QAction::triggered, this,
        &MainWindow::onActionDisconnectTriggered
    );
    connect(
        ui->actionAddSymbol, &QAction::triggered, this,
        &MainWindow::onActionAddSymbolTriggered
    );

    connect(
        ui->addTabButton, &QPushButton::clicked, this,
        &MainWindow::onAddTabButtonClicked
    );
    connect(
        ui->symbolTabs, &QTabWidget::tabCloseRequested, this,
        &MainWindow::onTabCloseRequested
    );
    connect(
        ui->symbolTabs, &QTabWidget::currentChanged, this,
        &MainWindow::onTabChanged
    );

    connect(
        ui->buyButton, &QPushButton::clicked, this, &MainWindow::onBuyClicked
    );
    connect(
        ui->sellButton, &QPushButton::clicked, this, &MainWindow::onSellClicked
    );

    connect(
        ui->modifyOrderButton, &QPushButton::clicked, this,
        &MainWindow::onModifyOrderClicked
    );
    connect(
        ui->cancelOrderButton, &QPushButton::clicked, this,
        &MainWindow::onCancelOrderClicked
    );
}
...
QWidget* MainWindow::createSymbolTab(const QString& symbol)
{
    ...
    ordersTable->setHorizontalHeaderLabels(
        {"Timestamp", "Order ID", "Side", "Price", "Quantity", "Status"}
    );
    ...
    orderActionsLayout->addSpacerItem(
    ...
    connect(
        modifyButton, &QPushButton::clicked, this,
        &MainWindow::onModifyOrderClicked
    );
    ...
    tradesTable->setHorizontalHeaderLabels(
        {"Timestamp", "Trade ID", "Price", "Quantity", "Source Order ID",
         "Matched Order ID"}
    );
    ...
    connect(
        sellButton, &QPushButton::clicked, this, &MainWindow::onSellClicked
    );
...
void MainWindow::removeSymbolTab(const int index)
{
    if (ui->symbolTabs->count() <= 1) {
        QMessageBox::information(
            this, "Cannot Close", "At least one symbol tab must remain open."
        );
        return;
    }
...
void MainWindow::onActionConnectTriggered()
{
    ...
    bool ok;
    const auto server = QInputDialog::getText(
        this, "Connect to Server", "Enter Host:Port", QLineEdit::Normal,
        "127.0.0.1:8080", &ok
    );
    ...
    const auto username = QInputDialog::getText(
        this, "Connect to Server", "Enter username", QLineEdit::Normal, "", &ok
    );
    if (!ok || username == "")
        return;
    ...
    auto invoke = [&](auto function) -> void {
        QMetaObject::invokeMethod(
            this, std::move(function), Qt::QueuedConnection
        );
    };

    m_client->register_handler<core::protocol::info::Info>(
        [invoke, this](const auto& msg) {
            invoke([this, msg] {
                handleInfo(msg);
            });
        }
    );
    m_client->register_handler<core::protocol::info::Error>(
        [invoke, this](const auto& msg) {
            invoke([this, msg] {
                handleError(msg);
            });
    ...
    m_client->register_handler<core::protocol::admin::LoginResponse>(
        [invoke, this](const auto& msg) {
            invoke([this, msg] {
    ...
    const core::protocol::admin::LoginRequest req{username.toUtf8().toStdString(
    )};
    m_client->request(req);
...
{
    bool ok;
    const auto symbol = QInputDialog::getText(
        this, "Add Symbol",
...
void MainWindow::onBuyClicked()
{
    ...
    const auto quantity =
        static_cast<core::Quantity>(widgets->qtySpinBox->value());
...
void MainWindow::onModifyOrderClicked()
{
    ...
    if (selectedItems.isEmpty()) {
        QMessageBox::information(
            this, "No Order Selected", "Please select an order to modify."
        );
        return;
    }

    const auto row = selectedItems.first()->row();
    const core::OrderId orderId =
        widgets->ordersTable->item(row, 1)->text().toULongLong();
    const auto side = widgets->ordersTable->item(row, 2)->text() == Buy
                          ? core::Side::Buy
                          : core::Side::Sell;
    const core::Price currentPrice =
        widgets->ordersTable->item(row, 3)->text().toULongLong();
    const core::Quantity currentQuantity =
        widgets->ordersTable->item(row, 4)->text().toULongLong();

    bool priceOk;
    const auto newPriceStr = QInputDialog::getText(
        this, "Modify Order", "Enter new price:", QLineEdit::Normal,
    ...
    bool qtyOk;
    const auto newQuantityStr = QInputDialog::getText(
        this, "Modify Order", "Enter new quantity:", QLineEdit::Normal,
    ...
    if (newPrice != currentPrice) {
        const auto reply = QMessageBox::question(
            this, "Confirm Action",
            "Price is being modified which will lead to a CancelAdd "
    ...
    const core::protocol::trading::ModifyOrderRequest req{
        getCurrentSymbol(), orderId, newQuantity
    };
    m_client->request(req);

    appendLog(
        QString("Sent ModifyOrderRequest for Order ID %1: New Quantity=%2")
            .arg(orderId)
            .arg(newQuantity)
    );
}
...
void MainWindow::onCancelOrderClicked()
{
    ...
    if (selectedItems.isEmpty()) {
        QMessageBox::information(
            this, "No Order Selected", "Please select an order to cancel."
        );
        return;
    }
...
void MainWindow::handleInfo(const core::protocol::info::Info& msg) const
{
    appendLog(QString("Received Info: %1").arg(msg.message().c_str()));
}

void MainWindow::handleError(const core::protocol::info::Error& msg)
{
    std::ostringstream os;
    os << msg.error_code();
    QMessageBox::warning(this, os.str().c_str(), msg.message().c_str());
}

void MainWindow::handleLoginResponse(
    const core::protocol::admin::LoginResponse& msg
)
{
    if (msg.success()) {
        QMessageBox::information(
            this, "Login successful",
            QString("Logged in with username: %1").arg(msg.username().c_str())
        );
        ...
        QMessageBox::critical(
            this, "Login failed",
            QString("Failed to login with username: %1")
                .arg(msg.username().c_str())
        );
    }
}

void MainWindow::sendAddOrderRequest(
...
)
{
    ...
    if (quantity == 0) {
        QMessageBox::warning(
            this, "Invalid Quantity", "Quantity cannot be zero."
        );
        return;
    }

    const core::protocol::trading::AddOrderRequest req{
...
{
    const core::protocol::trading::CancelOrderRequest req{
        getCurrentSymbol(), orderId
    };
    m_client->request(req);
...
void MainWindow::handleGetBookResponse(
    const core::protocol::view::GetBookResponse& msg
)
{
    ...
    if (const auto* widgets = getCurrentTabWidgets(); widgets) {
        const auto bestBid = msg.bids().empty()
                                 ? core::protocol::view::Level{0, 0}
                                 : msg.bids().front();
        const auto bestAsk = msg.asks().empty()
                                 ? core::protocol::view::Level{0, 0}
                                 : msg.asks().front();
        const auto spread = msg.bids().empty() || msg.asks().empty()
                                ? 0
...
void MainWindow::handleAddOrderResponse(
    const core::protocol::trading::AddOrderResponse& msg
) const
{
    appendLog(
        QString("AddOrderRequest ACCEPTED. Order ID: %1").arg(msg.order_id())
    );
    addOrderToTable(
        msg.order_id(), msg.side(), msg.price(), msg.quantity(), msg.timestamp()
    );
}

void MainWindow::handleModifyOrderResponse(
    const core::protocol::trading::ModifyOrderResponse& msg
)
{
...
void MainWindow::handleCancelOrderResponse(
    const core::protocol::trading::CancelOrderResponse& msg
)
{
    if (msg.success()) {
        appendLog(QString("CancelOrderRequest ACCEPTED for Order ID: %1")
                      .arg(msg.order_id()));
        removeOrderFromTable(msg.order_id());
    } else {
        appendLog(QString("CancelOrderRequest REJECTED for Order ID: %1")
                      .arg(msg.order_id()));
        QMessageBox::warning(
            this, "Cancel Rejected", "The order could not be canceled."
        );
    }
}
...
{
    appendLog(QString("Received Trade Notification. Price: %1, Quantity: %2")
                  .arg(msg.price())
                  .arg(msg.quantity()));
    addTradeToTable(
        msg.id(), msg.price(), msg.quantity(), msg.source_order(),
        msg.matched_order(), msg.timestamp()
    );

    if (auto* widgets = getCurrentTabWidgets())
        widgets->lastPriceLabel->setText(
            QString("Last Price: %1").arg(msg.price())
        );
...
void MainWindow::updateOrderStatusFromTrade(
    const core::OrderId orderId, const core::Quantity tradeQuantity
) const
{
    if (auto* widgets = getCurrentTabWidgets()) {
        for (auto i = 0; i < widgets->ordersTable->rowCount(); ++i) {
            if (const auto* item = widgets->ordersTable->item(i, 1);
                ...
                auto* qtyItem = widgets->ordersTable->item(i, 4);
                const core::Quantity remaining =
                    qtyItem->text().toULongLong() - tradeQuantity;
                qtyItem->setText(QString::number(remaining));
                if (remaining == 0)
                    widgets->ordersTable->item(i, 5)->setText("FILLED");
                break;
            }
        }
    }
...
{
    if (auto* widgets = getCurrentTabWidgets()) {
        widgets->logText->append(QString("[%1] %2").arg(
            QDateTime::currentDateTime().toString("HH:mm:ss"), message
        ));
    }
}

void MainWindow::updateBidsTable(
    const std::vector<core::protocol::view::Level>& bids
)
{
    if (const auto* widgets = getCurrentTabWidgets()) {
        ...
        for (auto i = 0; i < bids.size(); ++i) {
            const auto& l = bids[i];
            widgets->bidsTable->setItem(
                i, 0, new QTableWidgetItem(QString::number(l.price))
            );
            widgets->bidsTable->setItem(
                i, 1, new QTableWidgetItem(QString::number(l.quantity))
            );
            widgets->bidsTable->setItem(
                i, 2,
                new QTableWidgetItem(QString::number(l.price * l.quantity))
            );
        }
    }
}

void MainWindow::updateAsksTable(
    const std::vector<core::protocol::view::Level>& asks
)
{
    if (const auto* widgets = getCurrentTabWidgets()) {
...
void MainWindow::addOrderToTable(
    const core::OrderId orderId, const core::Side side, const core::Price price,
...
) const
{
    if (auto* widgets = getCurrentTabWidgets()) {
        ...
        widgets->ordersTable->setItem(
            row, 0, new QTableWidgetItem(formatTimestampMs(timestamp))
        );
        widgets->ordersTable->setItem(
            row, 1, new QTableWidgetItem(QString::number(orderId))
        );
        widgets->ordersTable->setItem(
            row, 2, new QTableWidgetItem(side == core::Side::Buy ? Buy : Sell)
        );
        widgets->ordersTable->setItem(
            row, 3, new QTableWidgetItem(QString::number(price))
        );
        widgets->ordersTable->setItem(
            row, 4, new QTableWidgetItem(QString::number(quantity))
        );
...
void MainWindow::removeOrderFromTable(const core::OrderId orderId) const
{
    if (auto* widgets = getCurrentTabWidgets()) {
        for (auto i = 0; i < widgets->ordersTable->rowCount(); ++i) {
            if (widgets->ordersTable->item(i, 1)->text().toULongLong() ==
                orderId) {
...
void MainWindow::addTradeToTable(
    const core::TradeId tradeId, const core::Price price,
    const core::Quantity quantity, const core::OrderId sourceOrder,
    const core::OrderId matchedOrder, const core::Timestamp timestamp
) const
{
    if (auto* widgets = getCurrentTabWidgets()) {
        ...
        widgets->tradesTable->setItem(
            row, 1, new QTableWidgetItem(QString::number(tradeId))
        );
        widgets->tradesTable->setItem(
            row, 2, new QTableWidgetItem(QString::number(price))
        );
        widgets->tradesTable->setItem(
            row, 3, new QTableWidgetItem(QString::number(quantity))
        );
        widgets->tradesTable->setItem(
            row, 4, new QTableWidgetItem(QString::number(sourceOrder))
        );
        widgets->tradesTable->setItem(
            row, 5, new QTableWidgetItem(QString::number(matchedOrder))
        );
    }
}
...
bool MainWindow::ensureConnected()
{
    if (!m_client) {
        QMessageBox::warning(
            this, "Not Connected", "Please connect to the server first."
        );
        return false;
    }
    return true;
