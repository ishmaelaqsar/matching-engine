...
public:
    ...
    // Helper for converting integral values to QString with locale-aware
    // formatting
    template <typename T> static QString qstr(T value)
    {
        static_assert(
            std::is_integral_v<T>, "qstr only supports integral types"
        );
...
private slots:
    ...
    void handleInfo(const core::protocol::info::Info& msg) const;
    void handleError(const core::protocol::info::Error& msg);
    void handleLoginResponse(const core::protocol::admin::LoginResponse& msg);
    void
    handleGetBookResponse(const core::protocol::view::GetBookResponse& msg);
    void handleAddOrderResponse(
        const core::protocol::trading::AddOrderResponse& msg
    ) const;
    void handleModifyOrderResponse(
        const core::protocol::trading::ModifyOrderResponse& msg
    );
    void handleCancelOrderResponse(
        const core::protocol::trading::CancelOrderResponse& msg
    );
    void handleTrade(const core::protocol::trading::Trade& msg) const;
    void updateOrderStatusFromTrade(
        core::OrderId orderId, core::Quantity tradeQuantity
    ) const;
...
private:
    ...
    void addOrderToTable(
        core::OrderId orderId, core::Side side, core::Price price,
        core::Quantity quantity, core::Timestamp timestamp
    ) const;
    void removeOrderFromTable(core::OrderId orderId) const;
    void addTradeToTable(
        core::TradeId tradeId, core::Price price, core::Quantity quantity,
        core::OrderId sourceOrder, core::OrderId matchedOrder,
        core::Timestamp timestamp
    ) const;
    void sendAddOrderRequest(
        core::Price price, core::Quantity quantity, core::Side side
    );
