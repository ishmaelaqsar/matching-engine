...
TEST(BookTest, BookAdd)
{
        ...
        const auto expected_bids = {std::make_pair<Price, Quantity>(103, 10), std::make_pair<Price, Quantity>(100, 5)};
        assert_iterable<std::pair<Price, Quantity>>(
                expected_bids, bids,
                [](const std::pair<Price, Quantity> &p1, const std::pair<Price, Quantity> &p2) -> void {
...
TEST(BookTest, BookModify)
{
        ...
        // modify 10@100 -> 5@100
        {
                const auto [success, trades] = book.modify_order(1, 100, 5, 3);
                ASSERT_TRUE(success);
                ASSERT_TRUE(trades.empty());
        }
        ...
        // modify 10@101 to 10@100 should fill buys
        {
                const auto [success, trades] = book.modify_order(2, 100, 10, 4);
                ASSERT_TRUE(success);
                ASSERT_EQ(trades.size(), 1);
        ...
        const auto expected_asks = {std::make_pair<Price, Quantity>(100, 5)};
        assert_iterable<std::pair<Price, Quantity>>(
                expected_asks, asks,
                [](const std::pair<Price, Quantity> &p1, const std::pair<Price, Quantity> &p2) -> void {
                        ASSERT_EQ(p1.first, p2.first);
